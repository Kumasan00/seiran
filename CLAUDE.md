...
citation （model, config に依存（resolve は知らない）。参照定義ファイル（references.toml / .json）の
          ...
          `NodeMap<CitationSiteFacts>` + CompiledCitationStyle から NodeId をキーにする表示 side table と
