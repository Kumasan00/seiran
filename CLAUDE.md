...
- `unwrap_used` / `expect_used` は**有効化していない**（restriction lint で `all` にも `pedantic` にも含まれない）。したがって `#[allow(clippy::unwrap_used)]` は何も抑制せず、書いてはいけない（上の「有効化されていない lint を allow しない」）
