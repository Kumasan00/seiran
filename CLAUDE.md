...
| P2  | 必須引数は `{}` で明示・個数順序固定（コマンドも `^` / `_` も）、コマンド名は英数字 | G1     |
...
cargo run -- build [-c <config_path>] [-v|-vv|-vvv|-q] [--log-file <path>]   # 設定ファイルの sources から PDF を生成（-v 工程 / -vv 内部詳細 / -vvv 最大 / -q 端末抑止 / --log-file はログ・warning・サマリをファイルへも出す）
