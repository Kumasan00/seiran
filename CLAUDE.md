...
| `read_style`      | `config/style.toml` の読み込み（`serde(default)` でデフォルト値マージ、`garde` 派生によるバリデーション）。`Style { core, extended }` の 2 層構造で、lowering/pdf_gen が読む `core` フィールド（`font_size` / `line_height_factor` / `background_color` / `heading` / `text` / `list` / `table` / `figure` / `equation` / `math` / `counters` / `header` / `footer`）と、現状未参照の `extended`（`footnote` / `toc` / `hyperref` / `reference`）に分離。`header` / `footer` は共通の `RunningContentStyle`（左中右スロット・トークン `{page}` `{pages}` `{title}` `{author}` `{date}`） |
