...
| `read_style`      | `config/style.toml` の読み込み（`serde(default)` でデフォルト値マージ、`garde` 派生によるバリデーション）。`Style { core, extended }` の 2 層構造で、lowering/pdf_gen が読む `core` フィールド（`font_size` / `line_height_factor` / `background_color` / `heading` / `text` / `list` / `table` / `figure` / `equation` / `math` / `counters` / `header` / `footer`）と `extended`（`footnote` / `toc` / `hyperref` / `reference`）に分離（`extended.reference.title` は `citation` が書誌見出し文字列として参照。残りは現状未参照）。`header` / `footer` は共通の `RunningContentStyle`（左中右スロット・トークン `{page}` `{pages}` `{title}` `{author}` `{date}`） |
...
| `citation`        | `\cite` の CSL 整形ステージ（parser の後・lowering の前）。`process_citations` が `InlineNode::Cite` をドキュメント順に走査し、`hayagriva`（`archive` feature 内蔵ロケール + `citationberg` で `.csl` 解析）で引用ラベルを採番（`[1][2]…`）して `label` を確定、引用された文献の書誌（References 見出し + 段落群）を本文末尾に追加。`bridge`（`read_references::Reference` → `hayagriva::Entry` 変換）/ `render`（`BibliographyDriver` 駆動・`ElemChildren` → `InlineNode` 変換）サブモジュール構成。初版は引用/書誌ともプレーン文字列（斜体等は段階対応） |
