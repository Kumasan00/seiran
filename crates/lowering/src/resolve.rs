...
//!
//! pass1（[`crate::lower_nodes`] の主走査）が発行した `LayoutNode::Ref` プレースホルダを、
//! pass1 完了時点で確定している [`crate::counter::CounterRegistry`] の labels テーブルで解決する。
...
///
/// `{of}` 解決後は隣接する同一スタイルの `Text` と結合する（[`crate::template::expand_template`] が
