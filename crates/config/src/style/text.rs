...
///
/// `punctuation_spacing` は JIS X 4051 のアキ調整（既定 `true`）。和文約物（括弧・句読点・中点）の
/// 前後アキ（連続約物の重複アキ詰め・行頭行末の版面揃え・両端揃えの収縮点化）に加え、和文文字と
/// 欧文文字（ラテン・ギリシャ・キリル・数字）が直接隣接する境界の四分アキ（和欧文間アキ）を行う。
