//! ページ組版の挙動スタイル設定型。
//!
//! 「同じ本文 + 同じ用紙で見た目だけ差し替える」対象のうち、段組み（[`crate::style::columns`]）とは別の
//! ページ単位の組版挙動を集約する。用紙サイズ・余白は物理設定として `config.toml` 側に残り、本型は
//! `style.toml`（`[page]`）に置く。実際の組版判断は `typeset::breaking::break_pages` が担い、本型はそのフラグだけを保持する。
...
/// 伸縮アキへ配分する（`typeset::breaking::break_pages`）。最終ページ・強制改ページ直前・伸縮アキの無いページは
/// 揃えない（自然高のまま）。
