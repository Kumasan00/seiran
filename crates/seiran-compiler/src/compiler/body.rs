...
use crate::typeset::{BodyLayout, BodyLayoutError, BodyLayoutInput, DocumentContent};
...
/// `resolve::analyze` が呼び出し元で既に完了しているため、ここでは失敗しない）。
pub(super) fn typeset_body(
  ...
  let run_pass = |footnote_numbers: Option<&[u32]>| {
    return run_body_pass(ctx, content, image_resources, footnote_numbers);
  };
...
fn run_body_pass(
  ...
  return match crate::typeset::layout_body(&input, content, footnote_numbers, resolve_images) {
    Ok(layout) => Ok(layout),
