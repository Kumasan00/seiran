...
//! `LayoutNode`（物理的なレイアウト表現）に変換する `typeset` クレートの module です。
//!
//! ## アーキテクチャ上の位置づけ
//!
//! ```text
...
/// Lowering のコンテキスト
///
/// 変換中に必要なスタイル設定（`config::Style`）への参照を保持します。
///
...
impl<'a> LoweringContext<'a> {
  ...
  /// # Arguments
  ///
  /// * `style` - `config::read_style()` の結果への参照
  #[must_use]
  pub fn new(style: &'a ReadStyle) -> Self {
    return LoweringContext {
      ...
      // （config::ImageConfig::default と同値）。テストはこの既定をそのまま使う。
      image_max_dpi: 300,
