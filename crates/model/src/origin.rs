//! 複数ソースをまたぐ処理の起源識別子 [`SourceId`] / [`Origin`]
//!
//! `typeset::lowering` が複数ソースファイルを 1 回の呼び出しでまとめて処理する際、各グループが
//! どのソース由来か（または CSL 整形ステージ等が合成した実ソースに属さないノードか）を表す。
//! `model` は診断ライブラリに依存しないため（#203）、`NamedSource` への逆引きは消費側
//! （`seiran::build_pdf`）が担う。
...
/// 診断・エラーが帰属する起源
///
/// 実ソースファイル由来なら `Source`、CSL 整形ステージ等が合成したノード由来なら `Generated`。
/// 「配列範囲外の `SourceId`」のような暗黙の sentinel でこの2つを区別しない
/// （合成グループも実ソース配列に連結して扱っていた旧実装の問題点、#259）。
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum Origin {
  /// 実ソースファイル由来
  Source(SourceId),
