...
- **どの環境・コマンドが verbatim か、数式内のコマンドが必須引数を何個取るかという語彙は `syntax` が持たない**
  — `ModeResolver`（環境本体・コマンド引数・数式内の引数個数の 3 本の関数）経由で `evaluator` のレジストリが
  単一の真実源になる（ユーザは変更できない ＝ P1 ガード）。引数モードは**引数の位置ごと**に引くので、同じ
  コマンドでも位置によってモードが違いうる。
  ...
  優先するので、数式内でも verbatim 宣言が効く（数式の語彙に無いコマンドは下の個数の打ち切りを受けないので、
  `$\code{...}$` も生読みされてから評価器が未知として拒否する）
...
- **`\begin{name}` の後ろの `{...}` を必須引数として読むのはテキスト本体の環境だけ**。テキスト本体では裸の
  `{` を書けない（P4）ので引数以外の読みが立たず、個数は評価器（`arity`）が環境名つきで診断する。数式本体では
  `{...}` は数式グループなので本体の先頭になる（#732）。必須引数を取る環境は無いので個数はパーサーが持たない
  （初めて足すときに個数をレジストリから引く形へ変える）
- **数式内のコマンド呼び出しは、数式の語彙が宣言する必須引数の個数（`evaluator::math` の `MathCommandKind`）を
  読んだら打ち切る**。後ろの `{...}` は数式グループになる（`$\alpha{b}$`、#753）。個数の宣言はパーサーの打ち切りと
  評価器の個数検査の両方が従う 1 箇所。テキスト内と数式の語彙に無いコマンドは後ろに続く `{...}` をすべて引数として
  読み、余分は評価器が診断する
- **テキスト・数式どちらの文脈でも 1 要素を読む規則は `parse_element` 1 つだけ**。`$...$`・数式グループ・
  数式環境の本体・引数の各ループは終端と「閉じないまま終わった」診断（`UnclosedInlineMath` /
  `UnclosedMathGroup` / `UnclosedDelimiter`）だけを持つ。数式内の `\begin` も環境として読み、数式内で
  使えないことは評価器が診断する（書いた位置で診断が変わらない、#688）
...
- 任意引数 `[...]` のエントリ分割は CST のトークン単位（`parse_key_value_options`）で、直下の構造トークン
  `,` がエントリ、各エントリの最初の構造トークン `=` が key / value の区切り。`\,` `\=`（`Escaped`）と入れ子の
  ノードは値の文字として平坦化する。`\cite` のキー分割も同じく直下の構造トークン `,` で割る
  （`split_text_on_commas`）。平坦化した文字列を割る形へ戻さない（エスケープの区別が消える。#687 / #731）
- 任意引数の検査（未知キー・同一組内のキー重複・値の型・値域）は 1 箇所（`opt_args`）が担う。ハンドラは
  ...
  長さの字面は `Length` の `FromStr` を config / style と共有し、`opt_args` / `widths` / `\space` は値域だけを
  上に重ねる（書式の解釈を frontend に複製しない。#690）
...
**収まり判定は 1 箇所**: 「このブロックは現在のリージョン（段）に収まるか」は、実配置と keep-with-next の
判定（見出しの孤立防止）が同じ規則を通る。段落は `paragraph_plan` の計画（脚注予約・自前の脚注・
widow / orphan 補正込み）、それ以外のブロックはカーソル値 `RegionCursor` の判定メソッドで決める。実効下限
（`page_limit` − 脚注予約）を選ぶ引数はどの判定にも無い — `RegionCursor` の判定メソッドは下限を
`RegionCursor::region_limit` で自分で導き、段落の計画が受け取るのは生の `page_limit` と脚注予約込みのカーソルだけ
（行の自前の脚注を足した下限は計画の中で導く）。
keep-with-next の判定はこれらを空回しして `starts_region` を読むだけで、下限や行送りを自前で導き直さない
（導き直すと脚注予約のあるリージョンで実配置と食い違い、見出しがページ末尾に残る。#686）。
...
- `test_support`: compiler 配下のテストが共有する fixture builder `TestProject`。組み立てた入力を production と
  ...
  規則で登録）と設定上書きの規則（config.toml・style.toml とも生 TOML テーブルへの差分 1 系統）は `//!`。
  字面のまま登録する画像の例外・型付きの config / style 並行実装（`Style` の `Serialize` を含む）を再導入しない
...
image not in manifest / 未対応の画像拡張子・フォントの `fvar` と軸指定の整合・`fvar` の可読性（レコードが
ファイル範囲外を指す破損を含む。保証点は `typeset::font::validation`。#681 / #725））は持たない — 同じ検査を
2 箇所に持つと、どちらが真の保証点か読めなくなるため（保証点は `publication` 節の表。#378）。
...
- **フォント調査の終了コードと部分結果**。一覧を書き切った / 受け手が先に終了した（`BrokenPipe`）→ 0、
  ...
  **調べ終えてから 1 度に書き出す** — 調査に失敗したら一覧を 1 行も出さずに診断エラーにする。診断はすべて
  ...
  compiler のフォント検証も同じ判定で、ビルドとサブコマンドで食い違わない。#725）。
