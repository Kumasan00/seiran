...
2. 対象 diff を取得する（指示があればその範囲、なければ `git diff main...HEAD` と
   未コミット変更の両方）。
3. 変更行を中心にチェックする。主な観点:
   - 返り値の `return` 明示（末尾式による暗黙返却の禁止）
   - モジュール・構造体・関数の日本語ドキュメントコメント
   - `mod.rs` 禁止（親は `foo.rs` + 子は `foo/<child>.rs`）、分割判断基準への適合
   - `use` 文: `*` 回避・明示インポート
   - エラー型: thiserror + miette 派生、日本語メッセージ、`code` の階層化、
     garde バリデーションの集約（詳細は error-handling skill 準拠）
   - 公開 API の維持（切り出した型の `pub use` 再エクスポート）
   - テストの AAA パターン
