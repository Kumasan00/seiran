...
- 探索の起点に迷ったら、データフロー（input（`project` / `style` の読込）→ `frontend` →
  `semantics` → `typeset::compose`（font → 画像パス収集 → 画像読込 → lowering → boxing（計測・画像寸法の確定）→
...
3. root facade の `pub use` re-export 越しの定義追跡・参照網羅など、grep の文字列一致では
   確定できない箇所に当たったら、推測で埋めず「ここから先は LSP が必要」と報告に明記する
   （LSP ツールはこのエージェントでは使えない。本体スレッドが LSP で辿る）。
4. 見つけた事実だけを報告する。推測で補完しない。見つからなければ
